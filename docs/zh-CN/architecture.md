...
- Windows 便携版更新优先通过 HTTPS 静态清单获取版本、资产大小和 SHA-256，清单不可用或校验失败时回退 GitHub Release API；每个渠道均先尝试当前代理，失败后再直连。下载完成后还会校验文件大小、SHA-256 和 PE 文件头，再由退出后的独立助手替换并在失败时恢复旧版本。
- 静态清单与更新包当前没有独立数字签名，SHA-256 不能抵御发布账户或清单同时被篡改；公开发布仍应增加 Windows 代码签名，并只通过 HTTPS 分发清单和资产。
