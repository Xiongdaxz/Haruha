...
通过当前用户注册表 `Internet Settings` 与 WinINet 刷新系统代理。应用流量统计在每次 Haruha 运行期间首次开启时通过 UAC 启动独立 Helper；关闭监控会停止 ETW 但保留空闲 Helper，因此同一运行期间再次开启不再提示 UAC。采集覆盖 TCP/UDP 和 IPv4/IPv6，排除回环并按可执行文件合并多进程。主要开发和打包链路已经在 Windows 上运行过；公开版本仍应按发布指南在干净机器复验 UAC 取消、采集异常、反复关开、休眠/恢复和退出清理。
